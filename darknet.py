...
def save_model():
    ...
    model = DarkNet(anchors).to('cuda:0')
    model.eval()
    x = torch.ones(1, 3, 320, 576).to('cuda:0')
    ys = model(x)
    for i, y in enumerate(ys):
        print(f'the {i + 1}th output shape is {y.shape}, output data is:\n{y}\n')
    torch.save(model, "workspace/tmp/jde_darknet.pth")

def inference_model():
    import struct
    print('cuda device count: ', torch.cuda.device_count())
    model = torch.load('workspace/tmp/jde_darknet.pth').to('cuda:0')
    model.eval()
    # print('model: ', net)
    ...
    f = open("workspace/tmp/jde_darknet.wts", 'w')
